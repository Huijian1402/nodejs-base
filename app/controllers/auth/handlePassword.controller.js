    ...
    if (!req.body.email) {
        ...
        User.findByEmail(req.body.email, (err, user) => {
            if (!user) {
                ...
                const resetPass = passgen(20,true,true);
                ...
                bcrypt.hash(resetPass, parseInt(process.env.BCRYPT_SALT_ROUND)).then((hashedPassword) => {
                    ...
                    mailer.sendMail(user.email, "Reset password", `Mật khẩu của bạn đã đổi thành ${resetPass} bấm vào link để đăng nhập ${process.env.APP_URL}/login với email của bạn là: ${user.email}`)
                })
